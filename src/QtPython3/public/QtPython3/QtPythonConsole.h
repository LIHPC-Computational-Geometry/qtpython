...
class QtPythonConsole : public QtTextEditor
{
	...
	/**
	 * @return	<I>true</I> si l'instance est actuellement dans l'attente d'un ordre de reprise (next/cont/run).
	 * @see		isRunning
	 ...
	 * Une instruction peut être exécutable (ou non t.q. commentaire), mono ou multiligne.
	 */
	class Instruction
	{
		public :
		...
		/**
		 * @return	<I>true</I> si l'instruction est exécutable, <I>false</I> dans le cas contraire.
		 */
		bool runnable ( ) const;
	...
	/**
	 * Fonction callback appelée lorsqu'une ligne du fichier python vient d'être exécutée. Actualise la console.
	 ...
	 */
	virtual void lineProcessedCallback (const std::string& fileName, size_t scriptLine, bool ok, const std::string& error);
	...
	/**
	 * Gestion de <I>logs</I> : commandes scripts et/ou sorties de commandes scriptées.
	 */
	//@{
	...
	class InstructionsFile
	{
		 ...
		 * @param		<I>true</I> si le script est exécuté en mode debug, <I>false</I> dans le cas contraire.
		 * @param		Prefix utilisé pour le nom du fichier
		 * @param		<I>true</I> si le jeu de caractères du fichier est <I>ISO-8859-15</I>, sinon <I>false</I>.
		 * @param		<I>true</I. si le fichier doit être détruit lors de la destruction de cette instance.
		 */
		InstructionsFile (bool debug, const std::string& filePrefix, bool iso8859, bool autoDelete);
		...
		/**
		 * @return		<I>true</I> si le script est exécuté en mode debug, <I>false</I> dans le cas contraire.
		 */
		 virtual bool debugMode ( ) const
		{ return _debug; }

		/**
		 * Détruit le fichier, libère la mémoire et réinitialise les différentes variables.
		 */
		virtual void reset ( );
		 ...
		 * @param		Numéro dans la console de la première ligne ajoutée au fichier
		 */
		virtual void addLines (const std::vector<std::string>& lines, size_t firstLineNum);
		...
		/**
		 * @return		Le nombre de lignes du fichier (hors lignes rajoutées par l'instance, type encodage des caractères).
		 * @see			getLineOffset
		...
		/**
		 * @return		Le nombre de lignes réel du fichier (incluant les lignes rajoutées par l'instance, type encodage des caractères).
		 * @see			getLineOffset
		 * @see			getLineCount
		 */	
		...
		/**
		 * @return		Le numéro de la première ligne de la console  ajoutée dans ces script.
		 */	
		virtual size_t getFirstLineNum ( ) const;

		/**
		 * @return		Le numéro de ligne dans le script correspondant au numéro de ligne dans la console donné en argument.
		 * 				Des lignes sont éventuellements rajoutées à celles de la console (support charset ISO-8859, ...).
		 * @see			getLineOffset
		 */
		virtual size_t getScriptLineNum (size_t consoleLineNum) const;

		/*
		 * @return		Le numéro de ligne dans la console correspondant au numéro de ligne dans le script donné en argument.
		 * @see			getLineOffset
		 */
		virtual size_t getConsoleLineNum (size_t scriptLineNum) const;

		/**
		 * @return		La ligne de numéro num dans la console donné en argument.
		 */
		virtual std::string getConsoleLine (size_t num) const;

		/**
		 * @return		Le nombre de lignes ajoutées au début (type d'encodage ISO, ...).
		 */
		...
		/**
		 * @param		<I>true</I> si le numéro de ligne donné est avec <I>offset</I>, <I>false</I> dans le cas contraire.
		 * @return		Le numéro de la première ligne exécutable à partir de celle transmise en argument, hors <I>offset</I>, ou 0
		 * 				si aucune ligne n'est exécutable. La numérotation commence à 1.
		 * @see getLineOffset
		 */
		virtual size_t nextRunnableLine (size_t after, bool withLineOffset) const;
	...
	/**
	 * Classe dont la vocation est de positionner le curseur en position éditable en fin de document lors de sa destruction.
	 */
	...
	/**
	 * @return	La ligne max atteinte. Peut différer de currentInstruction ( ) - 1 lorsqu'on est dans une boucle.
	...
	/**
	 * @return	La ligne d'instruction exécutable précédant la ligne courrante, ou 0 s'il n'y en a pas.
	 * @see		currentInstruction
	 */
	 ...
	 * @param	Si <I>true</I> s'arrête après la première instruction (permet de débuter avec la commande "suivant").
	 */
	...
	/**
	 * @return	Une conversion <I>C++</I> vers <I>Python</I> de l'instruction transmise en argument.
	 */
